...
const LEAVE_TYPES_LABELS: Record<string, string> = {
  lieu: '调休假', annual: '年假', sick: '病假', personal: '事假',
...
export default React.memo(function ToolCards({ message, confirmedDrafts, setConfirmedDrafts, isLoading, quickSend, onApprovalClick }: ToolCardsProps) {
      ...
      if (isLeave && leaveData) {
        return (
          <LeaveForm
            ...
            isConfirmed={confirmedDrafts.has(part.toolCallId)}
            isLoading={isLoading}
            recommendation={recommendation}
            balance={balance}
            onConfirm={({ type, startDate: sd, endDate: ed, reason: rs, note: nt }) => {
              const typeLabel = LEAVE_TYPES_LABELS[type] || type;
